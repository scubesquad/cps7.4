<?php require_once('global.php');
	...
	{
		$row_series = $db->get_row("SELECT series_branch_code FROM tb_cps_payorderseries WHERE series_branch_id = '".GetSQLValueString($_POST['ddlBranch'],'int')."' && series_id != ".GetSQLValueString($_REQUEST['id'],'int')."");
		if($row_series)
		{
	...
	else if(isset($_POST['txtlastno']) && !empty($_POST['txtlastno']) && $_POST['txtlastno'] != "") 
	{
		...
		{		
			$row_series = $db->get_row("SELECT series_branch_code FROM tb_cps_payorderseries WHERE series_branch_id = '".GetSQLValueString($_POST['ddlBranch'],'int')."' ");
			if($row_series)
			{
				echo '{"error":"true", "htmlcontent":"Cheque series for this branch is already exists"}';
				$db->closeDb();
	...
	if(isset($_REQUEST['do']) && ($_REQUEST['do'] == 'del') && isset($_REQUEST['id']) && !empty($_REQUEST['id']) ) {
		$db->query("delete from tb_cps_payorderseries where series_id  = ".$_REQUEST['id']."");
		echo '{"status":"true"}';
		$db->closeDb();
		exit();
	}	
