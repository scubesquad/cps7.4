...
class UploadTransactionController extends Controller
{
    ...
    public function getUploadRecordAjax(Request $req){
        $user = Auth::user();
        $uploads = UploadingData::whereNotNull('id');
        if (!empty($req->branch)) {
            // $uploads->where('branch', 'like', '%' . $req->branch . '%');
            $uploads->where('cps_branchmicr_code', $req->branch);
        }
        if (!empty($req->transaction_type)) {
            $uploads->where('cps_tr_code', $req->transaction_type);
        }
        if (!empty($req->book_size)) {
            $uploads->where('cps_book_size', $req->book_size);
        }
        // if($user->roles[0]->name != 'Super Admin' || $user->roles[0]->name != 'HO') {
        // //     $uploads->where('created_by',$user->adminid );
        // }
        $user = Auth::user();
        ...
        return DataTables::of($uploads)
            ->addColumn('action', function ($upload) {
                ...
                if(strpos($upload->cps_status, 'Rejected By') !== false){                    
                    $html = '';
                } else {
                    $html = '
    ...
    public function rejectPrintReuest(Request $req){
        ...
        if($req->ajax()){
            ...
            if($upload){
                return response()->json([
                    'status' => 200,
                    'message' => "Rejected request by ".$role
                ]);
            }
        }
    }
    ...
    public function manualStore(Request $request){
        ...
        if($request->account_type != 12 && $request->account_type != 16) {
            $rules['customer_ac_no'] = 'required';
        ...
        $accountCharges = AccountCharges::select('charges')->where('book_size',$request->book_size)->first();
        
        
        if($chequeSeries == null && $request->branch_name && $request->account_type) {
            $rules['cheque_series_not_exist'] = 'required';
        }

        // if($accountCharges == null && $request->book_size) {
        //     $rules['account_charges_not_exist'] = 'required';
        // }

        $validationMessage =  [
            'cheque_series_not_exist.required' => 'This Branch cheque series is not exist. Please add check series in cheque series master.',
        ];
        ...
        $unique = DB::select("SELECT MAX(cps_unique_req) as cps_unique_req
            FROM (SELECT cps_unique_req FROM tb_print_req_collection
                UNION ALL
                SELECT cps_unique_req FROM tb_uploadingdata
                UNION ALL
                SELECT cps_unique_req FROM tb_printque
                UNION ALL
                SELECT cps_unique_req FROM tb_pending_print_req) as t;");

        if($unique[0]->cps_unique_req > 0) {
            $uniqueNo = $unique[0]->cps_unique_req +1;
        } else {
            $uniqueNo='000001';
        }
        ...
        // add account charges
        if($accountCharges) {
            $charges = $accountCharges->charges * $request->no_of_books;
        } else {
            $charges = 0;
        }
        ...
        $uploadingData->cps_micr_account_no = substr($request->customer_ac_no, -6);
        ...
        $uploadingData->branch_sub_code = $request->branch_sub_code;
        ...
        $uploadingData->cheque_charges = $charges;
