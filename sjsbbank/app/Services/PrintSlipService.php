...
class PrintSlipService{
	...
	function printRequestSlip($pdf, $type, $firstchequedata, $secondchequedata, $thirdchequedata, $noOfRequestSlip, $noofbooks, $printersinfo, $trn_string_array, $bankDetails, $xpos, $ypos) {
        ...
        for ($i = 0; $i < $noOfRequestSlip; $i++) { 
            ...
            $pdf->SetXY($xpos['leftslipbranchaddress2'] +60, $yAxis + $ypos['leftslipbranchaddress2']+2);
            $pdf->Cell(100, 0, $branchDetails->branch_short_name, '', 0, 'L');
            ...
            if($arrSlipData[$i]['cps_tr_code'] != 12 && $arrSlipData[$i]['cps_tr_code'] != 16) {
            ...
            // $pdf->Image(public_path()."/image/checkbox.png", $xpos['rightslipcharacterstics']+35, $yAxis + $ypos['rightslipcharacterstics']);
            // $pdf->Image(public_path()."/image/checkbox.png", $xpos['rightslipcharacterstics']+48, $yAxis + $ypos['rightslipcharacterstics']);
            ...
            if (htmlspecialchars_decode($arrSlipData[$i]['cps_act_name']) == "SELF" && htmlspecialchars_decode($arrSlipData[$i]['cps_tr_code']) == 10 && substr(htmlspecialchars_decode($arrSlipData[$i]['cps_account_no']), 6, 1) != "2") {
                ...
                if($arrSlipData[$i]['cps_tr_code'] != 12 && $arrSlipData[$i]['cps_tr_code'] != 16) {
                    ...
                    $pdf->Cell(100, 5, "For " . $bankDetails->bank_name , 0, 0, 'R');
                }
