	...
	public function reprintCheque($req){
		...
		if ($resultnoofchequeleavestype) {
			...
			if (count($result) > 0) {
				...
				for ($j = 0; $j < count($firstrequestsliprow); $j++) {
					...
					if(isset($req->printinbunch) && $req->printinbunch == "bunch"){
						if(isset($req->requisition) && $req->requisition=1){
							$html .= $this->printSlipService->printRequestSlip($pdf, 2, $firstrequestslipdata, $secondrequestslipdata, $thirdrequestslipdata, 3, $noofbooks, $printersinfo, $transactionKey, $printerDetails);
		                    $isPrintSlip = 1;
						}
					}else{
					...
					if (!$requisitiononly) {
						...
						if ($selective) {
							if (in_array($p, $selective_page_array)) {
								$html .= $this->printChequeService->printCheques($pdf, 3, $firstchequedata, $secondchequedata, $thirdchequedata, 3, $printersinfo, $printerDetails);
							}
				...
				return [
	            	'status' => 203,
	            	'message' => "The cheques were printed successfully1."
	            ];
			}
		} else {
			return [
