...
class UserService
{
	...
	public function store($req){
		...
		$user->otp_verification = $req->otp_verification;
		...
		if($user->save()){
			if($req->branch_name != ""){
				$userBranch = new BranchUser;
				$userBranch->user_id = $user->adminid;
				$userBranch->branch_id = $req->branch_name;
				$userBranch->created_by = Auth::user()->adminid;
				$userBranch->created_at = now();
				$userBranch->save();
			}
	...
	public function update($req){
		...
		if($user->save()){
			//$userBranchExist = BranchUser::where('user_id', '=', $user->adminid)->where('branch_id', '=', $req->branch_name)->first();
			$userBranchExist = BranchUser::where('user_id', '=', $user->adminid)->first();
			if($userBranchExist) {
				$userBranchDelete = $userBranchExist::findOrFail($userBranchExist->id);
