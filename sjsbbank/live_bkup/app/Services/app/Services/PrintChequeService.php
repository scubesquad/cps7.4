...
class PrintChequeService{
	...
	function printCheques($pdf, $type, $firstchequedata, $secondchequedata, $thirdchequedata, $noofCheque, $printersinfo, $bankDetails) {
        $html = "";
        ...
        for ($i = 0; $i < $noofCheque; $i++) { 
            ...
            // to print address
            $pdf->SetXY($arrXaxisPos['branchX'], $yAxis+$arrYaxisPos["BankAddrY"]);
            $pdf->Cell(120, 0, $branchDetails->branch_name.": ".$branchDetails->branch_address1, '', 0, 'L');

            $address2 = $arrYaxisPos["BankAddrY"] + 2.5;
            $addressLine2="";
            ...
            $pdf->SetXY($arrXaxisPos['branchX'], $yAxis+$address2);
            $pdf->Cell(120, 0, $addressLine2, '', 0, 'L');
            // End of address

            
            // to print account number 
            $pdf->SetFont('Arial_B','B',10);
            $pdf->SetXY($arrXaxisPos['accnoX']-17.1, $yAxis+$arrYaxisPos["AcnoY"]);
            $pdf->Cell(100, 0, 'A/C No.', '', 0, 'L');
            
            $pdf->SetXY($arrXaxisPos['accnoX'], $yAxis+$arrYaxisPos["AcnoY"]);
            ...
            {
                $pdf->SetXY(0, $yAxis+$arrYaxisPos["Name"]+ 20);
            ...
            {
                $pdf->SetXY(0, $yAxis+$arrYaxisPos["Name"]);
            ...
            $pdf->SetFont('Arial','',5);
            $pdf->SetXY($arrXaxisPos['rotateDateTime'], $yAxis+$arrYaxisPos["VUniqReq"]-3);
            $pdf->StartTransform();
            $pdf->Rotate(90);
            ...
            $pdf->SetXY($arrXaxisPos['rotateDateTime'], $yAxis+$arrYaxisPos["VUniqReq"] - 20);
            $pdf->StartTransform();
            $pdf->Rotate(90);
            ...
            $pdf->SetXY($arrXaxisPos['rotateDateTime'], $yAxis+$arrYaxisPos["VUniqReq"] - 47);
            $pdf->StartTransform();
            $pdf->Rotate(90);
            ...
            $y = $yAxis+$arrYaxisPos["MICRY"];
            $pdf->SetXY($arrXaxisPos['chqseriesX'], $y);
            $pdf->Cell(150, 0, $chqseries, '', 0, 'L');

            $pdf->SetXY($arrXaxisPos['micr1X'], $y);
            $pdf->Cell(150, 0, $micr1, '', 0, 'L');

            if($arrChqData[$i]['cps_tr_code'] != 12){
                $pdf->SetXY($arrXaxisPos['micr2X'], $y);
                $pdf->Cell(150, 0, $micr2, '', 0, 'L');
            }
            $pdf->SetXY($arrXaxisPos['trncodeX'], $y);
