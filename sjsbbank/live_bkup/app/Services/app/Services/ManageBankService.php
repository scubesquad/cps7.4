...
class ManageBankService{
	...
	public function store($req){
		...
		$manageBank->subdomain = $subdomain[0];
		...
		$manageBank->sample_input_file = $this->uploadfile($req, $req->name, 'sample_input_file');
		$manageBank->output_file_format_description = $req->output_file_format_description;
		$manageBank->sample_output_file = $this->uploadfile($req, $req->name, 'sample_output_file');
		$manageBank->sample_requisition = $this->uploadfile($req, $req->name, 'sample_requisition');
		$manageBank->sample_cheque = $this->uploadfile($req, $req->name, 'sample_cheque');
	...
	public function uploadfile($req, $bank, $name){
		...
		if($req->hasFile($name)){
	        ...
	        $req->file($name)->storeAs('ManageBank/'.$bank.'/', $fileName, 'public');
	        $newName = 'ManageBank/'.$bank.'/'.$fileName;
	    }
	    return $newName;
	}
	...
	public function update($req){
		...
		$manageBank->sales_person = $req->sales_person;
		$manageBank->input_file_format_description = $req->input_file_format_description;
		$manageBank->sample_input_file = $this->uploadupdatefile($req, $req->name, 'sample_input_file', $manageBank->sample_input_file);
		$manageBank->output_file_format_description = $req->output_file_format_description;
		$manageBank->sample_output_file = $this->uploadupdatefile($req, $req->name, 'sample_output_file', $manageBank->sample_output_file);
		$manageBank->sample_requisition = $this->uploadupdatefile($req, $req->name, 'sample_requisition', $manageBank->sample_requisition);
		$manageBank->sample_cheque = $this->uploadupdatefile($req, $req->name, 'sample_cheque', $manageBank->sample_cheque);
		$manageBank->special_requirement = $req->special_requirement;
		$manageBank->printer_brand = $req->printer_brand;
		$manageBank->printer_model = $req->printer_model;
		$manageBank->os_name = $req->os_name;
