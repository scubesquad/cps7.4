...
class DatabaseBackupController extends Controller
{
    ...
    public function getDatabaseBackup(){
        ...
        // get bank details and crate folder for that bank if not exists
        $bank = $this->commonService->getBankName();

        // check if file not exist create new
        if(!Storage::disk('public')->exists('backup/'.$bank->bank_name)) {

            Storage::disk('public')->makeDirectory('backup/'.$bank->bank_name, 0775, true); //creates directory

        }

        $file_name = Storage::disk('public')->path('backup/'.$bank->bank_name.'/database_backup_on_' . date('d-m-Y_h-i-s') . '.sql');
