    ...
    <script type="text/javascript">
        ...
        $(document).ready(function(){
            ...
            $('body').on('click', '#save_single_leaf_modal_submit', function(e){
                ...
                $.ajax({
                    url: "{{ url('/process/reprint/update') }}",
                    type: "get",
                    data: {id:val},
                    success: function( response ) {
                        let selected_id_chkseries = [];
                        selected_id_chkseries = $('#schk').serialize();
                        // console.log(selected_id_chkseries);

                        // console.log("{{ url('/reprint-cheque?do=reprint&') }}"+selected_id_chkseries);
                        if($("#requisitionCheckbox").prop('checked') == true){
                            window.location = "{{ url('/reprint-cheque?do=reprint&"+selected_id_chkseries +"&requisition=1') }}";
                        }
                        else{
                            var newUrl = "{{ url('/reprint-cheque?do=reprint&') }}"+selected_id_chkseries;
                            // newUrl.replace('&amp;', '&');
                            link = newUrl.toString().replace('&amp;','&');
