<?php require_once('global.php');
	...
	{
		$row_series = $db->get_row("SELECT series_branch_code FROM tb_cps_ddseries WHERE series_branch_id = '".GetSQLValueString($_POST['ddlbranchid'],'int')."' && series_id != ".GetSQLValueString($_REQUEST['id'],'int')."");
		if($row_series)
		{
	...
	else if(isset($_POST['txtlastno']) && !empty($_POST['txtlastno']) && $_POST['txtlastno'] != "") 
	{
		...
		{		
			$row_series = $db->get_row("SELECT series_branch_code FROM tb_cps_ddseries WHERE series_branch_id = '".GetSQLValueString($_POST['ddlbranchid'],'int')."' ");
			if($row_series)
			{
				echo '{"error":"true", "htmlcontent":"Cheque series for this branch is already exists 1"}';
				$db->closeDb();
				exit();
			}
			else
			{
				$sql = "INSERT INTO tb_cps_ddseries (series_bank_id,series_branch_id,series_branch_code,series_from,series_to,series_lastno) VALUES ('".GetSQLValueString($_POST['ddlBank'],'int')."','".str_pad(GetSQLValueString($_POST['ddlBranch'],'int'), 3, "0", STR_PAD_LEFT)."','".GetSQLValueString($_POST['ddlbranchcode'],'int')."','".GetSQLValueString($_POST['txtFROM'],'int')."','".GetSQLValueString($_POST['txtTo'],'int')."','".GetSQLValueString($_POST['txtlastno'],'int')."')";
				echo 
				$db->query($sql);
				echo '{"status":"true"}';
				exit();
	...
	if(isset($_REQUEST['do']) && ($_REQUEST['do'] == 'del') && isset($_REQUEST['id']) && !empty($_REQUEST['id']) ) {
		$db->query("delete from tb_cps_ddseries where series_id  = ".$_REQUEST['id']."");
		echo '{"status":"true"}';
		$db->closeDb();
		exit();
	}	
