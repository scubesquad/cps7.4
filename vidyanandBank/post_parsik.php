...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
		 ...
		 //    if(!in_array($explode[29], $serialNoArr)){

		 //    array_push($serialNoArr, $explode[29]);
			// //	echo "select cps_unique_req from tb_print_req_collection where cps_account_no = ".trim($explode[5])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[29])." and cps_chq_no_to >= ".trim($explode[29]);
			// if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where cps_account_no = ".trim($explode[6])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[29])." and cps_chq_no_to >= ".trim($explode[29])."")) 
			// {
			...
			// 	break;
			// }
			// else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where cps_account_no = ".trim($explode[6])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[29])." and cps_chq_no_to >= ".trim($explode[29]).""))
			// {
			...
			// 	$entries_found = true;
			// 	break;
			// }else if($result = $db->get_results("select cps_unique_req from tb_printque where cps_account_no = ".trim($explode[6])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[29])." and cps_chq_no_to >= ".trim($explode[29]).""))
			// {
			// 	echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[6].". Cheque from series - ".$explode[29]."</p>";
			// 	$entries_found = true;
			// 	break;
			// }else if($result = $db->get_results("select cps_unique_req from tb_pending_print_req where cps_account_no = ".trim($explode[6])." and cps_tr_code = ".trim($explode[6])." and cps_chq_no_from <= ".trim($explode[29])." and cps_chq_no_to >= ".trim($explode[29]).""))
			// {
			// 	echo "<p style='color:red'>This file has records which are already uploaded for printing. Account No - ".$explode[6].". Cheque from series - ".$explode[29]."</p>";
			// 	$entries_found = true;
			// 	break;
			...
			// }else {
			// 	echo "<p style='color:red'>This file has duplicate serial numbers. Account No - ".$explode[6].". Cheque from series - ".$explode[29]."</p>";
			// 	$entries_found = true;
			// 	break;
			// }
		...
		if(!$entries_found)
		{
			...
			foreach ($data as $key => $explode) {
				...
				if($explode[7] !=12 ){
					...
					'$explode[23]', '$explode[21]', '$explode[22]','$branch_sub_code', '$explode[32]', '$explode[38]'
					)";
				} else {
