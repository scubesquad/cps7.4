...
$SearchString = "";
	...
	foreach ($result as $key => $row) {
		...
		$branch_micr_code = $row->cps_micr_code;
		...
		$data .= str_pad($branch_code, 6, "0", STR_PAD_LEFT).'        ';
		$data .= '00602'.$branch_sub_code.$pdate;
		$data .= str_pad($chq_pages, 12, " ", STR_PAD_LEFT);
		$data .= str_pad(1, 8, " ", STR_PAD_LEFT);
		// $data .= str_pad($no_of_books, 8, " ", STR_PAD_LEFT);
		$data .= str_pad($series_start, 12, " ", STR_PAD_LEFT);
		$data .= str_pad($instrument_type, 8, " ", STR_PAD_LEFT).'      ';   
		$data .= $cps_unique_req.'000'.$cps_pr_code. "\r\n";
		}
