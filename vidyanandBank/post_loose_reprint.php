...
{
	$sqlquery ="UPDATE tb_cps_loose_reprintque SET cps_reprint_approved = 1 WHERE id IN (".$_POST["ids"].") ";
	$db->query($sqlquery);
...
{
	$sqlquery ="delete from tb_cps_loose_reprintque WHERE id IN (".$_POST["ids"].") ";
	$db->query($sqlquery);
	echo '{"status":"true"}';
	$db->closeDb();
	exit();
