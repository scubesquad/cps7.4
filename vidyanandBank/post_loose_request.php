...
{
	$sqlcheckquery = "SELECT * FROM tb_cps_loose_reprintque WHERE pay_branch_id IN (SELECT pay_branch_id FROM tb_cps_loose where id IN (".$_POST["ids"]."))";
	...
	{
		$db->query("Insert Into tb_cps_loose_reprintque (pay_branch_id,cps_chq_no_from,cps_chq_no_to,cps_no_of_books,cps_book_size,cps_date,cps_process_user_id) SELECT pay_branch_id,cps_chq_no_from,cps_chq_no_to,cps_no_of_books,cps_book_size,cps_date,cps_process_user_id from tb_cps_loose where id IN (".$_POST["ids"].")");
	}
	echo '{"status":"true"}';
	$db->closeDb();
	exit();	
...
function FetchReprintRecords(){
	global $db;
	$id = 0;
	if($result = $db->get_results("SELECT * FROM tb_cps_loose WHERE cps_date BETWEEN '".date('Y-m-d',strtotime($_REQUEST['from_date']))."' and '".date('Y-m-d', strtotime($_REQUEST['to_date']))."'") )
	{	
		$tableinner = '';
		$tableheader = '<tr>
