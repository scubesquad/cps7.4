<?php
	...
	class db {
		...
		function __construct($dbuser, $dbpassword, $dbname, $dbhost) { //db
		...
		// ==================================================================
		//	Basic Query
		function query($query, $output = OBJECT, $insID = true)  {
			...
			if ( mysqli_error($this->dbh) ) {
				...
				if ( $this->result ) {
					...
					$i=0;
					$this->get_colCount = @mysqli_num_fields($this->result);
		...
		// function insertLog($table, $recordId, $userId, $actionPerformed, $details, $whereRecordName=""){
		// 	$date = date('Y-m-d h:i:s');
		// 	$json = json_encode($details);
		// 	// loop to rearrange the select record
		...
		// 		if($whereRecordName != "")
		// 		{
		// 			$sql .= " where ".$whereRecordName."=".$recordId;
		// 		}

		// 		$data = $this->get_row($sql);

		// 		foreach($details as $key => $detail){
		// 			$prevJson .= $key.":".$data->$key.",";
		// 		}
		// 		$prevJson = substr($prevJson, 0, -1);
		// 	}
		// 	$prevJson .= "}";
		// 	// return $prevJson;
		// 	$query = "INSERT INTO log_table(table_name, record_id, user_id, action_performed, previous_details, details, created) VALUES('$table', '$recordId', '$userId', '$actionPerformed', '$prevJson', '$json', '$date')";
