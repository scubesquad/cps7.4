	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		if(!$entries_found)
		{
			foreach ($data as $key => $explode) {
				...
				$branch_sub_code = substr($explode[6], 3, 3);
				$cps_product_code = substr($explode[6], 6, 3);
				if(empty($explode[20])){
					$explode[20]=0;
				}
				...
				cps_dly_bearer_order,cps_chq_no_from,cps_chq_no_to,cps_date, cps_micr_account_no, cps_process_user_id, cps_product_code, 
				...
				'Y', '$explode[29]','$explode[30]','$date_printed', '$explode[5]', '$cpsprocess_user_id', '$cps_product_code', 
