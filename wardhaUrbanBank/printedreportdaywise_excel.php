...
require_once(ROOT_CLASSES.'PHPExcel.php');
	...
	if(isset($_REQUEST['branchcode']) && !empty($_REQUEST['branchcode'])){
		$parts = explode("#", $_REQUEST['branchcode']);
		$condation .= ' AND prc.cps_branchmicr_code = "'.$parts[0].'" AND  prc.branch_sub_code="'.$parts[1].'"';
...
{
	$parts = explode("#", $_REQUEST['branchcode']);
	$sql1 = "select branch_name from  tb_branchdetails where branch_sub_code= ".$parts[1];
