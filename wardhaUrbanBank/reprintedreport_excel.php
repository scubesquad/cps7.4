...
$title = "";
	...
	$title = 'Printed Report for the period : ';
	if(isset($_GET['frm']) && !empty($_GET['frm']) && isset($_GET['to']) && !empty($_GET['to'])){
	$title .= date('Y-m-d',strtotime($_REQUEST['frm'])).' To '.date('Y-m-d',strtotime($_REQUEST['to']));
	}
...
if($result = $db->get_results($sql)){
	...
	$wrsheet->SetCellValueByColumnAndRow(4, 2, 'BRANCH NAME');
	$wrsheet->SetCellValueByColumnAndRow(2, 2, 'TRANSACTION TYPE');
	$wrsheet->SetCellValueByColumnAndRow(3, 2, 'CUSTOMER NAME');
