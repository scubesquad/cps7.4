...
if(isset($_GET['account_no']) && !empty($_GET['account_no'])){
	$condation .= ' AND cps_account_no LIKE "%'.$_GET['account_no'].'%"';
}
if(isset($_GET['cps_atpar']) && !empty($_GET['cps_atpar']))
{
	$condation .= " AND cps_tr_code = ".$_GET['cps_atpar'];
}

if(isset($_GET['cps_book_size']) && !empty($_GET['cps_book_size']))
{
	$condation .= " AND cps_book_size = ".$_GET['cps_book_size'];
}

if(isset($_GET['customername']) && !empty($_GET['customername'])){
	$condation .= " AND cps_act_name LIKE '%".$_GET['customername']."%'";
}

if(isset($_GET['ddlBranchName']) && !empty($_GET['ddlBranchName']))
{
	$condation .= " AND cps_branchmicr_code = '".$_GET['ddlBranchName']."' ";
}

if(isset($_GET['type']) && !empty($_REQUEST['type']) && $_REQUEST['type'] == 'search'  && 
	 isset($_REQUEST['frm']) && !empty($_REQUEST['frm']) &&  
	 isset($_REQUEST['to']) && !empty($_REQUEST['to']) ) {

	$sql = "select tb_reprint_req_collection.*, bd.branch_name from tb_reprint_req_collection LEFT join tb_branchdetails bd on bd.branch_code = tb_reprint_req_collection.cps_branchmicr_code where tb_reprint_req_collection.cps_date between '".date('Y-m-d',strtotime($_REQUEST['frm']))."' and '".date('Y-m-d', strtotime($_REQUEST['to']))."'".$condation;
} else {
	$sql = "select tb_reprint_req_collection.*, bd.branch_name from tb_reprint_req_collection LEFT join tb_branchdetails bd on bd.branch_code = tb_reprint_req_collection.cps_branchmicr_code";
}
