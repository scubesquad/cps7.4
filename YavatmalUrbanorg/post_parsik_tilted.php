	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
		   ...
		   // print_r($explode);
			...
			if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where cps_tr_code = '$explode[7]' and ((cps_chq_no_from <= '$explode[29]' AND cps_chq_no_to >= '$explode[29]') OR (cps_chq_no_from <= '$explode[30]' AND cps_chq_no_to >= '$explode[30]'))"))
			{
				echo "<p style='color:red'>This file has records which are already printed. | TR Code : ".$explode[7]." | Series : ".$explode[29]." to ".$explode[30]."</p>";
				$entries_found = true;
				break;
			}
			else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where cps_tr_code = '$explode[7]' and ((cps_chq_no_from <= '$explode[29]' AND cps_chq_no_to >= '$explode[29]') OR (cps_chq_no_from <= '$explode[30]' AND cps_chq_no_to >= '$explode[30]'))"))
			{
				echo "<p style='color:red'>This file has records which are already printed. | TR Code : ".$explode[7]." | Series : ".$explode[29]." to ".$explode[30]."</p>";
				$entries_found = true;
				break;
